...
--  Safe to re-run (uses IF NOT EXISTS / ON CONFLICT / DROP POLICY).
-- ============================================================
...
-- Organization info (single row, id always = 1)
...
-- ----------------------------------------------------------------
-- 2) SEED DATA (only inserts if the tables are empty)
-- ----------------------------------------------------------------

insert into public.org (id, short, name_full, tagline, about, story, place, email, stats)
select 1,
  'BHI',
...
where not exists (select 1 from public.org where id = 1);

insert into public.activities (title, description, date, images, created_at)
select * from (values
...
) as v(title, description, date, images, created_at)
where not exists (select 1 from public.activities);

insert into public.members (name, role, bio, sort)
select * from (values
...
) as v(name, role, bio, sort)
where not exists (select 1 from public.members);

insert into public.milestones (year, title, description, sort)
select * from (values
...
) as v(year, title, description, sort)
...
-- public can view images; only logged-in users can upload/change/delete
